...
class EpiMaxLoss(nn.Module):
    ...
    def __init__(self, alpha=5.5, beta=2.0, gamma=2.8):    
...
def run_experiment():
    ...
    # Standardization parameters for MNIST
    ...
    # Extract Scenarios
    print("Preparing the scenarios...")
    img_8, img_3, img_shoe = None, None, None
    ...
    # img_ambiguous = 0.5 * img_8 + 0.5 * img_3
    img_ambiguous2 = 0.5 * img_8 + 0.7 * img_3
    # Overlay two images, keeping the brightest pixels (full contrast)
    # img_ambiguous = torch.max(img_8, img_3)
    img_ambiguous = torch.max(img_8, img_ambiguous2)
    
    # ---------------------------------------------------------
    # NEW BLOCK: Visualize the input stimuli
    # ---------------------------------------------------------
    def denormalize(tensor):
        """Reverts normalization for clean visualization."""
        img_np = tensor.squeeze().cpu().numpy()
        return np.clip((img_np * std) + mean, 0, 1)

    fig_inputs, axes_inputs = plt.subplots(1, 4, figsize=(18, 5), dpi=380)
    
    axes_inputs[0].imshow(denormalize(img_8), cmap='gray', interpolation='nearest')
    axes_inputs[0].set_title("Clear digit '8'", fontsize=19)
    axes_inputs[0].axis('off')
    
    axes_inputs[1].imshow(denormalize(img_3), cmap='gray', interpolation='nearest')
    axes_inputs[1].set_title("Clear digit '3'", fontsize=19)
    axes_inputs[1].axis('off')
    
    axes_inputs[2].imshow(denormalize(img_ambiguous), cmap='gray', interpolation='nearest')
    axes_inputs[2].set_title("Ambiguous '3' vs '8'", fontsize=19)
    axes_inputs[2].axis('off')
    
    axes_inputs[3].imshow(denormalize(img_shoe), cmap='gray', interpolation='nearest')
    axes_inputs[3].set_title("OOD shoe", fontsize=19)
    axes_inputs[3].axis('off')
    
    plt.suptitle("Input stimuli for epistemic analysis", fontsize=23, weight='bold')
    plt.tight_layout()
    plt.show()
    # ---------------------------------------------------------
    ...
    fig, axes = plt.subplots(1, 3, figsize=(22, 7), dpi=380)
    ...
    for idx, (title, img) in enumerate(scenarios):
        ...
        # Annotate zones
        ax.text(0.2, 0.8, 'Axiomatic void\n($N > \Pi$)', color='black', alpha=0.6, ha='center', fontsize=15, weight='bold')
        ax.text(0.8, 0.8, 'Certainty', color='green', alpha=0.6, ha='center', fontsize=17, weight='bold')
        ax.text(0.8, 0.2, 'Ignorance /\nConflict', color='orange', alpha=0.8, ha='center', fontsize=17, weight='bold')
        ax.text(0.2, 0.2, 'Rejection', color='blue', alpha=0.5, ha='center', fontsize=17, weight='bold')

        # Plot the 10 classes
        scatter = ax.scatter(pi, N, c=range(10), cmap='tab10', s=100, edgecolors='k', zorder=5)
        
        # Add text labels for the classes
        for i in range(10):
            jitter_y = np.random.uniform(-0.02, 0.02) if N[i] < 0.05 else 0
            ax.annotate(str(i), (pi[i], N[i] + jitter_y), textcoords="offset points", xytext=(0,10), ha='center', fontsize=15, weight='bold')

        ax.set_title(title, fontsize=21, weight='bold')
        ax.set_xlabel(r'Possibility ($\Pi$)', fontsize=19)
        if idx == 0:
            ax.set_ylabel(r'Necessity ($N$)', fontsize=19)
            
        ax.set_xlim(-0.05, 1.05)
        ax.set_ylim(-0.05, 1.05)
        ax.grid(True, alpha=0.3)

    plt.suptitle("EpiMax epistemic space: 10-class distribution analysis", fontsize=25, weight='bold', y=1.05)
    plt.tight_layout()
    plt.show()
